...
1. The Beta distribution is a continuous distribution but his support is defined on the range $[0, 1]$, so it is not convinient to use it because $\lambda$ belongs to the range $[0, +\infty)$. Moreover, the Beta distribution is usually associated with the Bernoulli distribution.
...
$p(\lambda | X) \propto p(X | \lambda) \cdot p(\lambda) \propto$
...
$\propto \prod_{i = 1}^{n}(\lambda^{x_i}) \cdot e^{-n \cdot \lambda} \cdot \lambda^{\alpha - 1} \cdot e^{\beta \cdot \lambda} = $
$= \lambda^{(s + \alpha) - 1} \cdot e^{-(n + \beta) \cdot \lambda}$
