...
```{r Ex 1.0, message = FALSE, warning = FALSE, echo  = FALSE}
...
omega_nlm <- nlm(log_lik_weibull_rep, c(0, 0), hessian = T, data = y)
omega_nlm
theta(omega_nlm$estimate)
diag(solve(omega_nlm$hessian))

# Variance of the thetahat estimatorusing optimHess
...
**The $Wald \space confidence$ interval with level  $1−\alpha$ is defined as:**
...
where $\hat{\beta}_{\gamma}$ is the *constrained* MLE for $\beta$, with $\gamma$ fixed. 

```{r Ex 2.0, message = FALSE, warning = FALSE, echo = FALSE}
...
```{r Ex 2.0.1, warning = FALSE, message = FALSE, echo = FALSE}
...
Below we plotted them together to see the difference between the two confidence intervals.

```{r Ex 2.2, warning = FALSE, message = FALSE, echo = FALSE}
plot(function(x) -log_lik_weibull_profile_v(data=y, x) + weib.y.mle$value, 
     from = 0.1, to = 15,
     xlab = expression(gamma),
     ylab = 'profile relative log likelihood',
...
abline(h = -log_lik_weibull_profile_v(y, lrt.ci1[1]) + weib.y.mle$value,lty = 'dashed', col= "green")
...
points(lrt.ci1[1], -log_lik_weibull_profile_v(y, lrt.ci1[1]) + weib.y.mle$value, pch = 16, col = "green", cex = 1)
points(lrt.ci1[2], -log_lik_weibull_profile_v(y, lrt.ci1[2]) + weib.y.mle$value, pch = 16, col = "green", cex = 1)

segments(lrt.ci1[1], -8.1, 
         lrt.ci1[2], -8.1,
         col="green", lty = 1, lwd = 2)

text(7, -7.7, "95% Wald CI", col = "green")
...
segments(lrt.ci1[1],-qchisq(conf.level,1)/2, 
         lrt.ci1[1],-log_lik_weibull_profile_v(y, lrt.ci1[1]), col="red", lty = 2)

segments(lrt.ci1[2],-qchisq(conf.level,1)/2, lrt.ci1[2],
         -log_lik_weibull_profile_v(y, lrt.ci1[2]), col="red", lty = 2)

points(lrt.ci1[1], -qchisq(0.95,1)/2, pch = 16, col = "red", cex = 1)
points(lrt.ci1[2], -qchisq(0.95,1)/2, pch = 16, col = "red", cex = 1)


segments( lrt.ci1[1], -6.1, 
          lrt.ci1[2], -6.1, col="red", lty = 1, lwd = 2)

text(7, -5.7, "95% Deviance CI", col = 2)
...
#contour plot
contour(beta, gamma, t(llikvalues) - max(llikvalues),
       levels = -qchisq(conf.levels, 2)/2,
       xlab = expression(gamma),
       labels = as.character(conf.levels),
       ylab = expression(beta))
...
#  uniroot(function(x) n/x - n * log(beta) + sum(log(data)) - sum((data/beta)^x * log(data/beta)), c(1e-5,15))$root
#}

#curve(gamma.beta(data = y, x), from = 100, to = 200, n = 100, add = TRUE)
#points(weib.y.mle$par[2], weib.y.mle$par[1])
...
lrt.ci1 <- uniroot(function(x) -log_lik_weibull_profile_v(data = y, x) + weib.y.mle$value + qchisq(conf.level,1)/2,
                 c(100, weib.y.mle$par[2]))$root
...
segments( lrt.ci1[1], -9.9, 
          lrt.ci1[2], -9.9, 
          col="red", lty = 1, lwd = 2)

text(156.2, -9.2, "95% Deviance CI", col = 2)
